...
class Gaussian(logfileparser.Logfile):
    ...
    def after_parsing(self):
        ...
        # CJS scancoords will be added during parsing
    ...
    def extract(self, inputfile, line):
        ...
        if line[1:23] == "Optimization completed":
            ...
            # CJS added to capture scan optimzation energies
            # this will be the last coords and scf energy parsed before 
            # the optimization completion
            if hasattr(self, 'nscans') and hasattr(self, 'scfenergies'):
                if not hasattr(self, "scanenergies"):
                    self.scanenergies = []
                if not hasattr(self, "scancoords"):
                    self.scancoords = []

                self.scanenergies.append(self.scfenergies[-1])
                self.scancoords.append(self.atomcoords[-1])
        ...
        if line[1:21] == "Optimization stopped":
            ...
            # CJS added to capture scan optimzation energies
            # this will be False if the optimization is stopped 
            if hasattr(self, 'nscans') and hasattr(self, 'scfenergies'):
                if not hasattr(self, "scanenergies"):
                    self.scanenergies = []
                if not hasattr(self, "scancoords"):
                ...
                self.scanenergies.append(False)
                self.scancoords.append(False)
        ...
        # CJS this is no longer correct for finding PES
        ...
        # CJS updated scan identification
        if line[1:33] == 'Number of optimizations in scan=':
            self.set_attribute('nscans', int(line.split('=')[1]))        
        ...
        # CJS added basis set descriptor lookup
        # Has to deal with lines like:
        # Standard basis: 6-311+G(d,p) (5D, 7F)
        if line[1:16] == "Standard basis:":
            #print('hi')
            basis_descript = line.split(':')[1][1:-1]
            self.set_attribute('basis_descript', basis_descript)
...
#                                                     0      554.3796303       -0.05152700                
        ...
        # CJS zero-point correction
        if "Zero-point correction=" in line:
            self.set_attribute('zeropt_energy', utils.convertor(float(line.split()[2]), "hartree", "eV"))
